...
currentdir=`pwd`
parameters=`getopt -o tThH -a -l template,help,template-title:,template-description:,dumponly,importguild -- "$@"`
...
do
    ...
    case "$1" in
        ...
        -h | -H | --help)
            ...
            echo "  --dumponly: only dump guild info and do NOT create new guilds"
            echo "  --importguild: create new guilds usin' existin' guild info"
            echo "    in these two cases the unused auth and guildid can be pretty arbitrary but due to my shitty arse bash skills they shall not be omitted"
            echo
        ...
        --)
            ...
            if [ ! "$dumponly" ]
            then
                [ "$6" ] && targetguildid="$6" || targetguildid=`curl 'https://discordapp.com/api/v6/guilds' -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:75.0) Gecko/20100101 Firefox/75.0' -H 'Accept: */*' -H 'Accept-Language: en-US' --compressed -H 'Content-Type: application/json' -H "Authorization: $targetauth" -H 'Origin: https://discordapp.com' -H 'Connection: keep-alive' -H 'Referer: https://discordapp.com/channels/@me' -H 'Cookie: __cfduid=d5a33608e55b75b1fc02781bd0757b7bb1584254754; locale=en-US; __cfruid=88aeb55eb0a8d28a41017b02ece60520142c92f8-1586063772' -H 'TE: Trailers' --data '{"name":"Cocoganger Chat Central","region":"us-south","icon":null,"channels":null}' | grep -o '^{"id": "[0-9]*"' | grep -Eo "[0-9]*"`
            fi
            break
            ;;
        *)
...
[ ! "$importguild" ] && sourceguilddump && exit
if [ ! "$dumponly" ]
then
    changeguild
    processroles
