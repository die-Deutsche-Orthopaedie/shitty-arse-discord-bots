...
7a. or you can just use step 6's results and just replace `--data "XXXXXXXX"` with `-F "payload_json={\"content\":\"$1\",\"nonce\":\"$RANDOM$RANDOM$RANDOM$RANDOM$RANDOM$RANDOM\",\"tts\":false}" -F "filename=@$2"`, **and replace `-H "Content-Type: application/json"` with `-H "Content-Type: multipart/form-data; boundary=---------------------------$RANDOM$RANDOM"`**, it can also work
