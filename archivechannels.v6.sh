...
function analysis_subprcess { # $1 = process id
    ...
    do
        ...
        if [ `echo "$singlemessage" | sed 's/, "attachments"/\n, "attachments"/g' | grep -Eo '"attachments": \[.{1,}\], "embeds"' | head -1` ]
        then
            local singleattachment
            for singleattachment in `echo "$singlemessage" | sed 's/, "attachments"/\n, "attachments"/g' | grep -Eo '"attachments": \[.{1,}\], "embeds"' | head -1 | sed 's/"attachments": \[//g' | sed 's/\], "embeds"//g' | sed 's/}, {/}\n{/g'`
            do
