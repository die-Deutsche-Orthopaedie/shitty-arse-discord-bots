...
function batgeneration() {
    ...
    echo "aria2c -x 128 -s 128 -j 128 -k 1M -R -c --auto-file-renaming=false -i $exportfilename" >> "$batfilename"
    if [ "$remote" ]
    then
...
currentdir=`pwd`
parameters=`getopt -o aAe:E:u:U:r:R:hHfFnNcC -a -l antics,max-filesize:,threadlimit:,export-filename:,uber-pack:,uber-standalone:,remote:,rclone,help,force-pack,nitro,oneindex-optimization:,checksums -- "$@"`
...
do
    ...
    case "$1" in
        ...
        -h | -H | --help)
            ...
            echo
            echo "Usage: "
            echo "./discordfilehosting.v6.sh [options] path/remotepath channelid configfilepath [curlparameters]"
            echo
            echo "Options: "
            ...
            echo "  -c or -C or --checksums: redownload and do checksums after uploadin' files to discord to make sure the file is uploaded as is, recommended for unstable connections or large data uploads"
...
then
    ...
    discordlink=`uploadfile "über pack **$uber2.rar** ready to fire$leftfunny$rightfunny" "/tmp/$uber2.rar" "$processid"`
...
then
    ...
    rm "direct.download.bat" "drag.list.file.and.download.bat" -f
    discordlink=`uploadfile "über pack **$uber.rar** ready to fire$leftfunny$rightfunny" "/tmp/$uber.rar" "$processid"`
    echo -e "your über pack's link: \e[36m$discordlink\e[0m"
    rm "/tmp/$uber.rar" -f
fi
